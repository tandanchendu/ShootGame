...
exports.onAfterBuild = function (options, result) {
    ...
    let cmd = `node ../updateVersion/version_generator.js -v 1.0.0 -u http://${getIPAdress()}:${remote_port}/ -s ${path.join(result.dest, resdir)} -d ${path.join(Editor.Project.path, "assets")}`    
    console.warn(cmd);
